...
const MovieFilterBar: React.FC<MovieFilterBarProps> = ({ onSubmit }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      onSubmit(values);
      dispatch(sortMovies({field: values.sort, order: values.order})); // replace the existing dispatch line with this
    },
    onReset: () => {
      onSubmit({ year: "", type: "", sort: "" });
    },
  });
  ...
  <form onSubmit={formik.handleSubmit} className="flex mt-4 items-center">
    <div className="flex flex-grow items-center">
      ...
      {formik.values.year || formik.values.type || formik.values.sort ? (
        <Button
          variant="light"
          size="small"
    ...
    <div className="flex-none mx-2">
      ...
      {formik.touched.sort && formik.errors.sort ? (
        <div className="text-red-500 text-xs italic">{formik.errors.sort}</div>
      ) : null}
    </div>
    <div className="flex-none mx-2">
  <select
  ...
  {formik.touched.order && formik.errors.order ? (
    <div className="text-red-500 text-xs italic">{formik.errors.order}</div>
  ) : null}
</div>
